...
| `formelay_ratelimit_outbound_wait_seconds` | histogram | `form`, `target` | Time spent waiting for an outbound rate-limit token. `target` is either a channel `id` or the literal `spam_filter`. Only observed once actual waiting happened (`rate_limit.on_limit: wait`, see [configuration.md](configuration.md#rate_limit-optional) for channels or [configuration.md](configuration.md#spam_filterrate_limit-optional) for the spam filter) — a channel or spam filter with no `rate_limit` configured, or one whose first check already passed, never appears here. |
